...
const Projects = () => {
	...
	return (
		<div
			id="Projects"
			className="w-full min-h-screen flex flex-col px-[5%] lg:px-[10%] py-10"
		>
			<div className="flex gap-2 font-bold text-4xl md:text-5xl justify-center">
				<p
					data-aos="fade-up"
					data-aos-delay="200"
					className="pb-3 inline-block bg-custom-linear-cyan bg-clip-text text-transparent"
				>
					Projects
				</p>
			</div>
				...
				<TabsContent value="project">
					...
					>
						{project.map((item, i) => (
							<Card
								key={i}
								className="w-full px-5 h-96 group bg-transparent border-cyan-700/60 border shadow-2xl p-6"
							>
								...
								<p className="text-gray-300 flex-wrap line-clamp-2">
									{item.description}
								</p>
								<div className="flex justify-between mt-2">
									<Link
										href={item.link ?? ""}
										className="text-blue-400 hover:underline flex gap-1 justify-center items-center"
									>
										<ExternalLink className="size-5" />
										Live App
									</Link>
									<Button
										className="text-white/70 hover:cursor-pointer hover:scale-105"
										variant="gradient"
										asChild
									>
				...
				<TabsContent value="tech-stack">
					...
					>
						{techStack.map((item, i) => (
							<Card
								key={i}
								className="w-40 h-48 p-8 group bg-transparent border border-cyan-700/60 hover:scale-105 duration-300 transition-all"
							>
								...
								<p className="font-bold text-center text-lg text-white/70">
									{item.name}
								</p>
							</Card>
						))}
